...
pub(crate) enum Command {
    ...
    /// трогаем. У K-fold проверяются все folds, а при повторах — все повторы,
    /// поэтому проверка означает CV-оценку, а не модель одного произвольного
    /// fold.
    CheckCandidate {
        data: PreparedData,
...
pub(crate) enum Event {
    ...
    Epoch {
        phase: Phase,
        /// Номер разбиения: у holdout всегда 0, у CV растёт (при повторах — до
        /// `k × повторов`). Живая кривая по нескольким разбиениям невозможна —
        /// номера эпох повторяются.
        fold: usize,
        epoch: usize,
        loss: f32,
    ...
    TrainDone {
        ...
        /// Разброс R² между повторами разбиения; `None` там же, где и
        /// `r2_std_folds`, и 0 у CV без повторов.
        r2_std_repeats: Option<f32>,
