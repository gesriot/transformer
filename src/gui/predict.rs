    ...
    pub(super) fn ui_predict(&mut self, ui: &mut egui::Ui) {
        ...
        match info {
            ...
            Some(info) => {
                ...
                // Что именно получится, говорим до запуска: для .xlsx это
                // копия исходной книги, для остальных источников — новая
                // книга без оформления.
                ui.label(
                    "Вход .xlsx и выход .xlsx: результат — копия исходной книги, в которой \
                     данные изменены только на выбранном листе; книге с формулами предписывается их \
                     пересчёт при открытии. Для остальных источников (.csv, .xlsm, .ods) \
                     результат — НОВАЯ книга только со значениями: стили, формулы и другие \
                     листы не сохраняются.",
                );
                if ui
                    .add_enabled(
