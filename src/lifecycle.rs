...
//!
//! Память живёт в процессе, но не теряется безвозвратно: загрузка checkpoint с
//! секцией происхождения возвращает раскрытие обратно. Общего реестра нет,
//! поэтому файл, которого нет под рукой, ни о чём не свидетельствует.
