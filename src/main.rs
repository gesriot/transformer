...
fn print_export_summary(output: &str, summary: &ExportSummary) {
    ...
    if summary.preserved {
        // Про пересчёт здесь молчим: он нужен только книгам с формулами, и
        // обещать его всегда значило бы описывать не тот файл. Подробности —
        // в manual.
        println!("  исходная книга сохранена: данные изменены только на выбранном листе");
    } else {
        println!(
