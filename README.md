...
Данные делятся на train / validation / test (по умолчанию holdout 70/15/15); на
малых выборках — K-fold, при необходимости повторённый несколькими нарезками
pool. Архитектура, lr, число эпох и порог прунинга подбираются **только по
...
- подготовка CSV/TSV/текстовых таблиц и Excel в `.tnum` (лист книги выбирается
  явно: `--sheet` в CLI, список в GUI);
...
- диагностика, предупреждения об экстраполяции и экспорт таблицы с прогнозами
  (исходная `.xlsx` сохраняется: данные меняются только на выбранном листе,
  формулам предписывается пересчёт);
